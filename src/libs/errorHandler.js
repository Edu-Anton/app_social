...
exports.procesarErroresDeBD = (err, req, res, next) => {
    if (err instanceof mongoose.Error || err.name === 'MongoError') {
        log.error('Ocurrió un error relacionado a moongose.', err);
        err.message = 'Error relacionado a la base de datos ocurrió inesperadamente. Para ayuda contacte a anton@mail.com';
        err.status = 500
    }
    next(err);
}
